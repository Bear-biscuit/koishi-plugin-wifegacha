...
function gxlp(ctx, config) {
  ...
  ctx.command("更新老婆 <name> <image> 更新老婆信息").action(async ({ session }, name2, image) => {
    ...
    (0, import_fs7.writeFileSync)(import_path8.default.join(wifegachaPath4, `${wifeComeFrom ? wifeComeFrom + config.wifeNameSeparator : ""}${name2}.png`), buffer);
    await ctx.database.set("wifeData", {
      name: name2
    }, {
    ...
    session.send([
      ...
      "老婆更新成功",
      import_koishi10.h.image((0, import_url5.pathToFileURL)(import_path8.default.join(wifegachaPath4, `${wifeComeFrom ? wifeComeFrom + config.wifeNameSeparator : ""}${name2}.png`)).href)
    ]);
  });
}
__name(gxlp, "gxlp");
...
function cmmlp(ctx, config) {
  ...
  ctx.command("重命名 <name> <newName> 重命名老婆").action(async ({ session }, name2, newName) => {
    ...
    (0, import_fs8.renameSync)(wifeData2[0].filepath, import_path9.default.join(wifegachaPath4, `${newName}.png`));
    await ctx.database.set("wifeData", {
    ...
    session.send([
      ...
      "老婆更新成功",
      import_koishi14.h.image((0, import_url7.pathToFileURL)(import_path9.default.join(wifegachaPath4, `${newName}.png`)).href)
    ]);
  });
}
__name(cmmlp, "cmmlp");
...
async function apply(ctx, config) {
  ...
  if (!(0, import_fs9.existsSync)(wifegachaPath3)) {
    ctx.logger.info("wifegacha文件夹不存在,开始初始化");
    (0, import_fs9.mkdirSync)(wifegachaPath3);
  }
