...
function nlp(ctx, config) {
  ctx.command("牛老婆 <userId> 牛指定群友老婆").action(async ({ session }, userId) => {
    ...
    const affection = myUserData.wifeHistories.find(
      (item) => item.wifeName === targetUserData.wifeName
    )?.affection ?? 0;
    const randomNumber = Math.floor(Math.random() * 100);
    let probabilityValue = 0;
    const successRate = utils_default.camelCase(
      lpNum,
      ntrSuccessCount,
      targetWifeNum,
...
function rlp(ctx, config) {
  ...
  ctx.command("日老婆 增加老婆好感度").action(async ({ session }) => {
    ...
    if (!config.fuckWifeSwitchgear) {
      session.send([(0, import_koishi7.h)("quote", { id: session.messageId }), "好感度功能未开启，请联系管理员"]);
      return;
    }
    if (config.fuckWifeBlockGroup.includes(session.channelId.toString())) {
      session.send([(0, import_koishi7.h)("quote", { id: session.messageId }), "本群好感度功能已被禁止，请联系管理员"]);
      return;
    }
    await utils_default.createUserData(ctx, session);
    await utils_default.createGroupData(ctx, session);
...
每级好感度会影响被牛老婆成功率` : ""}`,
          ...
          session.send([(0, import_koishi7.h)("quote", { id: session.messageId }), `好感度冷却中，剩余冷却${minutes}分${seconds}秒`]);
        }
      }
    } else {
...
var Config = import_koishi15.Schema.intersect([
  ...
  import_koishi15.Schema.object({
    ...
    probabilityMath: import_koishi15.Schema.union([
      import_koishi15.Schema.const(0).description("直接概率"),
      import_koishi15.Schema.const(1).description("特定算法")
    ]).role("radio").default(0).description("牛老婆成功率计算方式"),
    probabilityMathDirect: import_koishi15.Schema.number().default(50).min(0).max(100).step(1).description("直接概率(只有选择直接概率时有效)"),
  ...
  import_koishi15.Schema.object({
    divorceDateInterval: import_koishi15.Schema.number().default(10).description("离婚时间间隔(秒)"),
