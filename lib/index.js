...
if (import_path.default.join(__dirname).split("\\").pop() == "utils") {
  backgroundImagePath = import_path.default.join(__dirname, "../../../..", "data/assets/bj.png");
} else {
  backgroundImagePath = import_path.default.join(__dirname, "../../..", "data/assets/bj.png");
}
function ensureDirs() {
...
function prepareRenderMix(ctx, useColorNames, options = {}) {
  const {
    colorDir: colDir = colorDir,
    grayDir: grayOutDir = grayDir,
...
async function generateMixedBackgroundImage(ctx, colorImageNames, options = {}) {
  const {
...
function lptj(ctx, config) {
  ctx.command("老婆图鉴 [targetUserId] 查看老婆图鉴").action(async ({ session }, targetUserId) => {
    ...
    if (config.illustratedBook) {
      ...
      const imageBuffer = await sprit_default.generateMixedBackgroundImage(ctx, lpList);
      session.send([
...
var wifegachaVoicePath = import_path9.default.join(__dirname, "../../..", "data/assets/wifeVoice");
async function apply(ctx, config) {
  ...
  if (!(0, import_fs8.existsSync)(wifegachaVoicePath)) {
    ctx.logger.info("wifeVoice文件夹不存在,开始初始化");
    (0, import_fs8.mkdirSync)(wifegachaVoicePath);
  }
