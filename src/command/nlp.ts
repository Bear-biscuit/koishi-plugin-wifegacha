...
export function nlp(ctx: Context, config: Config) {
  ctx.command("牛老婆 <userId> 牛指定群友老婆").action(async ({ session }, userId) => {
    ...
    if(config.ntrBlockGroup.includes(session.channelId.toString())){
      session.send([h("quote", { id: session.messageId }), "本群牛老婆功能已被禁止，请联系管理员"]);
      return;
    }
    ...
    if(userId === session.userId) {
      session.send([h("quote", { id: session.messageId }), "自己牛自己，你真是个变态🤓"]);
      return;
    }
    // 创建目标用户数据
    ...
    if (myUserData.ntrCount >= config.ntrOrdinal) {
      session.send([h("quote", { id: session.messageId }), "你已经没有机会了"]);
      return;
    }
